...
with open("/Users/mvhsan/Downloads/mushroom.txt") as meshfile:
	meshfile.seek(0)
...
def sigmoid(x):
	return (math.pow(math.e, 20*x) + 1.25) / (math.pow(math.e, 20*x) + 1)

for i in range(len(points)):
	point = points[i]
	if point[1] < cutoff:
		points[i] = (point[0] * sigmoid(point[1] - cutoff), \
			cutoff + (point[1] - cutoff) / math.pow(1.25, 2))
	else:
		points[i] = (point[0] * sigmoid(point[1] - cutoff), point[1])
...
for i in range(len(points)):
	point = points[i]
	points[i] = (point[0], point[1] + (min_y - min_y2))
...
mesh.Export('neck_thick.stl', 'STL Format')
