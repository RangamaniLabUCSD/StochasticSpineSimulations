...
with open("/Users/mvhsan/Downloads/mushroom.txt") as meshfile:
	...
	for line in meshfile:
		values = line.split(" ")
		points.append((float(values[0])*math.pow(0.1, 1/3), 1.25*float(values[1])*math.pow(0.1, 1/3)))
...
for i in range(len(points)):
	point = points[i]
	if i < cutoff:
		points[i] = (point[0], point[1] + (max_y / 2) * (1 if i == 0 else i) / cutoff)
	else:
		points[i] = (point[0], point[1] + (max_y / 2))
